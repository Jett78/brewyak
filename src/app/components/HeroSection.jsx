    ...
    useGSAP(()=>{
        ...
        tl.from(stepsRef2.current, {
          ...
          tl.to(beergoodText.current, {
           ...
           tl.to(stepsRef2.current.children, {
            y:-200,
            duration:0.5,
            stagger:0.2,
            opacity:0,
  ...
  return (
   <main className='relative w-full ' ref={mainref}>
     <div ref={maincontainerref} className='py-[20em]'>
      ...
      <div>
        <h2 className='uppercase absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2  font-bold text-[90px] whitespace-nowrap' ref={beergoodText}>
        <span ref={whyText}>Why</span> Our beer is so good</h2>
      </div>

     {/* process container */}
      <div className='flex justify-center gap-[60px] absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2' ref={stepsRef} >
        {process.map((data,index) => (
          <div key={index} className='py-8 w-[250px] h-[290px] border rounded-lg grid gap-2 place-items-center text-center' >
             <figure className="bg-[#272361] w-[80px] h-[80px] rounded-full flex items-center justify-center">
                       <img src={data.img} alt="icons" />
                    </figure>
              <h2 className='font-medium text-[22px] text-secondwhite'>{data.title}</h2>
              <p className='w-[210px] font-regular text-[16px] text-thirdwhite'>{data.desc}</p>
          </div>
        ))}
      </div>

     {/* process container-2 */}
      <div className='flex justify-center gap-[60px] absolute top-1/2 left-1/2 -translate-y-1/2 -translate-x-1/2' ref={stepsRef2} >
